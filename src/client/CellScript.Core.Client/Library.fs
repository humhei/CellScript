...
type CellScriptClient<'Msg> = CellScriptClient of Client<'Msg>
with 
    /// internal helper function
    member internal x.Call message: Async<obj[,]> =     
        let (CellScriptClient client) = x
        client.RemoteServer <? message


[<RequireQualifiedAccess>]
module CellScriptClient =
    ...
    module private LambdaExpression =
        ...
        let ofUci (client: CellScriptClient<'Msg>) (uci: UnionCaseInfo) =
            let clientValue = Expr.Value client
            let callMethodInfo = typeof<CellScriptClient<'Msg>>.GetMethod("Call",BindingFlags.Instance ||| BindingFlags.NonPublic)
            ...
            let rec loop uciAccum uci =
                ...
                | Params (parameters) ->
                    ...
                    let callExpr = 
                        ...
                        Expr.Call(clientValue, callMethodInfo, args)
    ...
    let apiLambdas (client: CellScriptClient<'Msg>) =
        let tp = typeof<'Msg>
        if FSharpType.IsUnion tp then
            FSharpType.GetUnionCases tp
            |> Array.collect (LambdaExpression.ofUci client)
