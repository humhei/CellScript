...
with 
    ...
    member internal x.Call (actor: IActorRef<'Msg>) message: Async<obj[,]> =        /// internal helper function
        actor <? message
...
module Client =
    ...
    module private LambdaExpression =
        ...
        let ofUci (remoteActor: IActorRef<'Msg>) (client: Client<'Msg>) (uci: UnionCaseInfo) =
            let clientValue = Expr.Value client
            let remoteActorValue = Expr.Value remoteActor
            ...
            let rec loop uciAccum uci =
                ...
                | Params (parameters) ->
                    ...
                    let callExpr = 
                        ...
                        Expr.Call(clientValue, callMethodInfo, remoteActorValue :: arg)
    ...
    let apiLambdas (remoteActor: IActorRef<'Msg>) (client: Client<'Msg>) =
        let tp = typeof<'Msg>
        if FSharpType.IsUnion tp then
            FSharpType.GetUnionCases tp
            |> Array.collect (LambdaExpression.ofUci remoteActor client)
