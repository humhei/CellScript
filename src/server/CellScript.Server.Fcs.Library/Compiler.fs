...
module SerializableExcelReference =
    let getFsxPath (scriptsDir: string) (xlRef: SerializableExcelReference) =
        let addr = SerializableExcelReference.cellAddress xlRef
...
[<RequireQualifiedAccess>]
type CompilerMsg =
    | Eval of scriptsDir: string * xlRef: ExcelRangeContactInfo * code: string
...
module Compiler =
    ...
    type XlRefCache =
        { Id: int
          XlRef: SerializableExcelReference
        ...
        | Result.Ok dll ->
            useContext dll (fun assembly ->
                ...
                let paramValue = 
                    ...
                    | [| param |] -> 
                        let convertToParam (paramType: Type) (xlRef: ExcelRangeContactInfo) =
                            ...
                            | _ ->
                                ...
                                let paramValue = 
                                    ...
                                    | [| param |] ->
                                        ...
                                        | "System.Object[,]" ->
                                            box xlRef.Content
                                        | _ when param.ParameterType = typeof<ExcelRangeContactInfo> ->
                                            box xlRef
                                        | _ when param.ParameterType = typeof<FunctionSheetReference> ->
                                            let sheetReference: SheetReference = 
    ...
    let createAgent system: IActorRef<CompilerMsg> = 
        spawnAnonymous system (propsPersist (fun ctx ->
            ...
            let rec loop (state: Map<SerializableExcelReference, XlRefCache>) = actor {
                let! msg = ctx.Receive() 
                match msg with
                | CompilerMsg.Eval (scriptsDir, xlRef, code) ->

                    let xlRefPos = SerializableExcelReference.ofExcelRangeContactInfo xlRef
                    let script = SerializableExcelReference.getFsxPath scriptsDir xlRefPos
                    ...
                    let newState = 
                        try 
                            compileToNewByState state
                        with ex ->
                            let error: Result<string, string> = Result.Error ex.Message
